...
The map layer editor's **Attributes/Popup** tab can open a mini-dashboard as a floating modal whenever the user clicks a feature. Visualizations inside this modal reference the clicked feature's attributes via `${feature.<key>}` substitution — a separate syntax from the `${variable_name}` substitution used on the main dashboard.

1. Find the map grid item, click its three-dot menu, and click **Edit**.
2. In the map editor's **Layers** list, click the **China Flowlines** layer to open the layer editor.
3. Switch to the **Attributes/Popup** tab.
...
1. Click **+ (Add Item)** in the mini-dashboard toolbar.
...
7. Close the popup layout editor.
