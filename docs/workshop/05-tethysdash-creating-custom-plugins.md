...
Exit edit mode and zoom in on the map past zoom 12 until the flowlines render. Click any river segment in China — the GeoGLOWS Forecast Plot should immediately re-render with the forecast for that `comid`. Click a different segment and the plot updates again.
